...
(let* ((num-samples 100)
       ...
       (X (getf dataset :x))
       (y (getf dataset :y))
       (o ($adagrad-optimizer :lr 0.005))
       (layers (list ($affine-layer (* 2 output-dim) 128 :winit :he)
                     ($lkyrelu-layer :lky 0.001)
                     ($affine-layer 128 64 :winit :he)
                     ($lkyrelu-layer :lky 0.001)
                     ($affine-layer 64 output-dim)))
       (n ($snn layers :el ($mse-layer) :o o))
       (ntr 2000))
